	...
	return (
		<div>
			...
			<div className='event__item item w-full'>
				...
				<div
					className={`description max-w-3xl ${
						HasSpeakerIntro ? 'hasSpeakerIntro' : ''
					}`}
				>
					{HasSpeakerIntro ? (
						<div className='block md:flex'>
							<div className='flex mb-4'>
								<div
									className={`w-[9rem] md:w-[10rem] lg:w-[15rem] shrink-0 ${
										ImgCropped ? 'h-[9rem] md:h-[10rem] lg:h-[15rem]' : ''
									}`}
								>
									<img
										src={SpeakerImg}
										className={`w-full ${
											ImgCropped ? 'object-cover h-full' : ''
										}`}
										alt={speaker}
									/>
								</div>
								<h4 className='block md:hidden self-end ml-6 font-sans font-semibold text-h2'>
									{speaker}
								</h4>
							</div>
							<div className='md:ml-6'>
								<h4 className='hidden md:block mb-4 font-sans font-semibold text-h2'>
									{speaker}
								</h4>
								<div className='mb-4'>
