	...
	return (
		<>
			<div className='cta my-5 self-center'>
				<button
					className='border-2 border-secondary text-h4 text-white bg-secondary py-3 px-5 rounded-full font-gaya font-medium mr-4'
					onClick={toggleModal}
				>
					{sectionTitles[language].watchTrailer}
				</button>
				...
				<a href={venueLink} target='_blank'>
					<button className='border-2 border-secondary py-3 px-5 rounded-full text-h4 font-gaya font-medium'>
						{sectionTitles[language].buyTicket}
					</button>
				</a>
			</div>
			...
			{isTrailerOpen && (
				...
				<>
					...
					<div
						className={`gray ${isTrailerOpen && 'show'}`}
						onClick={toggleModal}
					></div>
					{/* // </div> */}
				</>
