...
export default async function HomeView({ language }) {
	...
	for (let film of films.records) {
		...
		const eventsOfFilm = filmEvents.filter((event) => {
			return event.fields?.Film?.[0] === filmId;
		});
	...
	return (
		<div id='content' className='relative'>
			<div className='w-full min-h-screen flex flex-col justify-center isolate relative z-[60]'>
				<LanguageSelect />
				<div className='py-10 mix-blend text-shadow font-puffling'>
					{/* <Dynamicp5TestTwo /> */}
					<h1
						className={`text-center text-h1 text-primary ${
